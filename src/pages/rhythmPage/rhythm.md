...
핵심 아이디어: 한 마디를 직접 16칸 백트래킹으로 채우기보다, “1박(4칸)을 완성하는 BeatPattern(미니 리듬)”들을 단계별로 미리 정의하고, 4개의 BeatPattern을 이어 붙여 마디를 만드는 방식입니다. 박 단위 조립을 쓰면 1~6단계처럼 ‘박 안에서만 복잡하게’ 만드는 정책을 자동으로 강제할 수 있고, 표기/학습 관점에서도 박의 착지감이 유지됩니다.

단계별 규칙 객체(StageRules): 각 단계는 (1) 허용되는 1박 패턴 후보군(beatPatterns)과 (2) 마디 전체에 대한 제약으로 정의합니다. 제약에는 예를 들어 16분이 많은 박의 최대 개수(maxDenseBeatsPerBar), 마디 내 쉼표 최소/최대 개수(minRestsPerBar/maxRestsPerBar), 점음표 허용 여부 및 마디 내 점박 최대 개수(allowDots/maxDottedBeatsPerBar), 그리고 후반 단계에서만 박 경계 넘어가는 이벤트 허용(allowCrossBeatEvents) 같은 옵션이 들어갑니다. 점음표는 length에 이미 실제 시간(예: 점8분=3, 점4분=6)을 반영하고, dots는 “모양 표시용”으로만 쓰는 전제를 유지합니다.
...
export interface StageRules {
  ...
  minRestsPerBar: number; // 마디 내 쉼표 이벤트 최소 개수
  maxRestsPerBar: number; // 마디 내 쉼표 이벤트 최대 개수
  allowDots: boolean;
...
function validateBar(events: RhythmEvent[], rules: StageRules): boolean {
  ...
  if (restCount < rules.minRestsPerBar || restCount > rules.maxRestsPerBar)
    return false;
...
// 다음 단계(5~8단계)는
// - beatPatterns에 16분 쉼표 위치 다양화, 혼합 밀도 증가,
// - dot 패턴(3+1, 6+2) 추가,
// - maxDenseBeatsPerBar / maxDottedBeatsPerBar 조절,
// - 8단계에서만 allowCrossBeatEvents=true 옵션 고려
// 식으로 확장하면 됩니다.
```
